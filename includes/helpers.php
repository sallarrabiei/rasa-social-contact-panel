<?php
/**
 * SSCP_Helpers — static utility class for sanitization, validation, and URL building.
 *
 * All methods are pure functions: no side-effects, no WordPress state dependencies
 * beyond the core sanitize_* functions that are available everywhere.
 */
...
class SSCP_Helpers {
	...
	/**
	 * Keeps only characters valid in international phone numbers.
	 */
	...
	/**
	 * Sanitize an integer and clamp it within [$min, $max].
	 */
	...
	/**
	 * Validate that $val is one of the whitelisted $allowed values.
	 */
	...
	/**
	 * Sanitize a sort-order integer: 1–99.
	 */
	...
	// -------------------------------------------------------------------------
	// URL builders (each platform has its own scheme/format)
	// -------------------------------------------------------------------------
	...
	public static function build_facebook_url( $value ) {
		// Accept full URL or page name/ID
	...
	public static function build_discord_url( $value ) {
		// Accept invite link or server/user ID
		if ( filter_var( $value, FILTER_VALIDATE_URL ) ) {
			return self::sanitize_url( $value );
		}
	...
	public static function build_wechat_url( $value ) {
		// WeChat deep links aren't standard on web; return a sanitized URL or empty.
		if ( filter_var( $value, FILTER_VALIDATE_URL ) ) {
			return self::sanitize_url( $value );
		}
		return '';
	...
	// -------------------------------------------------------------------------
	// Master dispatcher: given platform ID + raw value, returns ready-to-use URL
	// -------------------------------------------------------------------------

	public static function build_platform_url( $platform_id, $value, array $platform = [] ) {
		switch ( $platform_id ) {
			case 'whatsapp':  return self::build_whatsapp_url( $value, $platform['message'] ?? '' );
			case 'telegram':  return self::build_telegram_url( $value );
			case 'messenger': return self::build_messenger_url( $value );
			case 'instagram': return self::build_instagram_url( $value );
			case 'facebook':  return self::build_facebook_url( $value );
			case 'twitter':   return self::build_twitter_url( $value );
			case 'linkedin':  return self::build_linkedin_url( $value );
			case 'tiktok':    return self::build_tiktok_url( $value );
			case 'youtube':   return self::build_youtube_url( $value );
			case 'snapchat':  return self::build_snapchat_url( $value );
			case 'pinterest': return self::build_pinterest_url( $value );
			case 'discord':   return self::build_discord_url( $value );
			case 'skype':     return self::build_skype_url( $value );
			case 'viber':     return self::build_viber_url( $value );
			case 'line':      return self::build_line_url( $value );
			case 'wechat':    return self::build_wechat_url( $value );
			case 'email':     return self::build_mailto_url( $value );
			case 'phone':     return self::build_tel_url( $value );
			case 'sms':       return self::build_sms_url( $value );
			case 'website':   return self::build_generic_url( $value );
			default:          return self::build_generic_url( $value );
		}
	}

	/**
	 * Return placeholder text for the admin value input, contextual per platform.
	 */
